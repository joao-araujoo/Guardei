...
router.post("/url", async (req, res, next) => {
  try {
    const result = await captureUrlForUser(prisma, req.user.id, { ...req.body, origin: req.body?.origin || (req.captureTokenId ? "extension" : "quick-capture") });
...
router.post("/screenshot", async (req, res, next) => {
  try {
    const analysis = await analyzeScreenshot(req.body?.dataUrl);
    ...
    const video = await prisma.video.create({ data: {
      ...
      schemaVersion: 4,
      aiEngine: process.env.GEMINI_API_KEY ? "gemini-vision" : "local-fallback",
    } });
    await prisma.savedAsset.create({ data: {
      userId: req.user.id,
      videoId: video.id,
      kind: "screenshot",
...
router.post("/thought", async (req, res, next) => {
  try {
    const text = String(req.body?.text || "").replace(/[\u0000-\u001F]/g, " ").trim().slice(0, 8000);
    if (!text) return res.status(400).json({ ok: false, message: "Escreva alguma coisa para guardar." });
    const tags = Array.isArray(req.body?.tags) ? req.body.tags.map(item => String(item).trim().slice(0, 40)).filter(Boolean).slice(0, 10) : [];
    const thought = await prisma.quickThought.create({ data: { userId: req.user.id, text, contextUrl: String(req.body?.contextUrl || "").trim().slice(0, 2000) || null, tags } });
